...
public class Heap {
	...
	public boolean insert(int key, String name){
		if(currentSize == maxSize){
			return false; // means array is full
		}
	...
	public void trickleUp(int idx){
		...
		// loop as long as we havent reached the root and key of nodeToInset's parent is less than new node

		while(idx > 0 && heapArray[parentIdx].getKey() < nodeToInsert.getKey()){
			heapArray[idx] = heapArray[parentIdx]; // move parent down
			idx = parentIdx;
			parentIdx = (parentIdx - 1)/2; // moving up
	...
	private void trickleDown(int idx){
		...
		int largerChildIdx;
		Node top = heapArray[idx]; // save last into top vairable
		
		// while will rin as long as idx is not on the nottom row(at least 1 child)
		
		while(idx < currentSize/2){
			int leftChildIdx = 2 * idx + 1; // left chidld id position
	...
	public void displayHeap() {
        ...
        for(int m = 0; m < currentSize; m++) {
            if(heapArray[m] != null) {
                System.out.print( heapArray[m].getName() + " ");
            }
            System.out.println();
            ...
            while(currentSize > 0) {
                ...
                System.out.print(heapArray[j].getName());
                j++;
                if(j == currentSize) {
                    break;
                }
